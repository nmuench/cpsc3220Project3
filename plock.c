...
void plock_enter(plock_t *lock, int priority)
{
	...
	pthread_cond_init(&threadNode->waitCV, NULL);
	//printf("Reached here\n");
	//printf("1. Current head points to %p\n", lock->head);
	...
	{
		threadNode->next = lock->head;
		lock->head = threadNode;
		//printf("2. Current temp points to %p\n", temp);
		//printf("2. Current head points to %p\n", lock->head);
		//printf("--------------------------------");
	}
